...
from src.models import Jobs,SavedJobs,User
...
def cleanup_expired_job(db):
    ...
    unverified_accounts_deleted = db.execute(delete(User).where(User.is_verified == False,User.created_at < cutoff))
    deactivated_accounts_deleted = db.execute(delete(User).where(User.is_deleted == True,User.deleted_at < cutoff))
